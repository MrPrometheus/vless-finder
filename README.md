...
```bash
git clone https://github.com/vless-finder/vlessFinder.git
cd vlessFinder
...
- Обновит пути в `config.yaml`
- Создаст команду `vlessfinder` (доступна глобально)
- Зарегистрирует systemd сервис с автозапуском

После установки на экране будут показаны URL подписок для каждого пользователя.
...
```bash
systemctl stop vlessfinder
systemctl disable vlessfinder
rm /etc/systemd/system/vlessfinder.service
systemctl daemon-reload
rm -rf /opt/vlessfinder /var/lib/vlessfinder /var/log/vlessfinder
rm /usr/local/bin/vlessfinder
```
